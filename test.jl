...
let 
    ...
    for i = 1:1
        ...
        S_ave_arr, S_std_arr = scanMeasNoisyCAOneDepth(rule30NoisyStep,nAsites,nBsites,nmeas_start,nmeas_end,nmeas_step,pertb_start,pertb_end,pertb_step,nsteps,nstB)

        nmeas_l = floor.(Int,collect(range(nmeas_start,stop=nmeas_end,step=nmeas_step)))
        pertb_l = collect(range(pertb_start,stop=pertb_end,step=pertb_step))

        save2DData(nmeas_l,pertb_l,S_ave_arr,S_std_arr,string("data/240405/240405_",idx_start+i))
    end
end
